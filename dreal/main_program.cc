...
void MainProgram::AddOptions() {
  opt_.overview = fmt::format("dLinear {} : delta-complete SMT solver",
                              get_version_string());
  ...
  auto* const simplex_sat_phase_option_validator =
      new ez::ezOptionValidator("s4", "in", "1,2");
  ...
  auto* const lp_solver_option_validator =
      new ez::ezOptionValidator("t", "in", "soplex,qsoptex");
  ...
  auto* const verbose_simplex_option_validator =
      new ez::ezOptionValidator("s4", "in", "0,1,2,3,4,5");
...
#pragma STDC FENV_ACCESS ON
    ...
    if (precision == 0) {
      cerr << "\n"
           << "ERROR: --precision can't be set to zero (maybe try "
              "--exhaustive?).\n"
           << "\n"
           << "In order to support problems that may contain strict "
              "inequalities, dLinear4\n"
           << "reduces the precision value (or delta) by a small amount, and "
              "any strict\n"
           << "inequalities are de-strictified before being sent to the "
              "simplex solver.  For\n"
           << "this reason, the precision must be strictly positive.\n"
           << "\n"
           << "If you are sure that your problem contains no strict "
              "inequalities (not just in\n"
           << "the asserted formulas themselves, but in any conjunctive clause "
              "derived from\n"
           << "them), or if you simply wish to run the algorithm to "
              "completion, use\n"
           << "--exhaustive instead, and the precision value will be set to "
              "zero (but strict\n"
...
#pragma STDC FENV_ACCESS DEFAULT
  ...
  if (opt_.isSet("--exhaustive")) {
    ...
    DREAL_LOG_DEBUG(
        "MainProgram::ExtractOptions() --precision = {} (--exhaustive)",
        config_.precision());
  }
  ...
  if (opt_.isSet("--simplex-sat-phase")) {
    ...
    config_.mutable_simplex_sat_phase().set_from_command_line(
        simplex_sat_phase);
  ...
  if (opt_.isSet("--lp-solver")) {
    ...
    Config::LPSolver val =
        lp_solver == "qsoptex" ? Config::QSOPTEX : Config::SOPLEX;
